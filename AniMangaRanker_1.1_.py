...
class AniMangaRankerApp:
    ...
    def setup_gui(self):
        ...
        # ------------------ Options: List Statuses and Image Loading ------------------
        options_frame = tk.Frame(self.control_frame, bg="grey14")
        options_frame.pack(fill="x", pady=5)
        tk.Label(
            options_frame,
            text="Select list statuses to pull entries from:",
            font=("Arial", 12),
            bg="grey14", fg="snow"
        ).pack(anchor="center", pady=(0,5))

        # Create one frame for all checkboxes; arranged horizontally and centered.
        cb_frame = tk.Frame(options_frame, bg="grey14")
        cb_frame.pack(anchor="center", pady=5)
        # Configure grid columns to have equal weight.
        for i in range(5):
            cb_frame.columnconfigure(i, weight=1)

        # Create BooleanVars
        self.var_completed = tk.BooleanVar(value=True)
        self.var_watching = tk.BooleanVar(value=False)
        self.var_paused = tk.BooleanVar(value=False)
        self.var_dropped = tk.BooleanVar(value=False)
        self.var_load_images = tk.BooleanVar(value=False)

        # Status Checkbuttons (placed in one row)
        tk.Checkbutton(cb_frame, text="Completed", variable=self.var_completed,
                       font=("Arial", 12), bg="grey14", fg="snow",
                       activebackground="grey28", activeforeground="snow",
                       selectcolor="grey28", indicatoron=True).grid(row=0, column=0, padx=5, pady=5)

        tk.Checkbutton(cb_frame, text="Watching (Anime)/Reading (Manga)", variable=self.var_watching,
                       font=("Arial", 12), bg="grey14", fg="snow",
                       activebackground="grey28", activeforeground="snow",
                       selectcolor="grey28", indicatoron=True).grid(row=0, column=1, padx=5, pady=5)

        tk.Checkbutton(cb_frame, text="Paused", variable=self.var_paused,
                       font=("Arial", 12), bg="grey14", fg="snow",
                       activebackground="grey28", activeforeground="snow",
                       selectcolor="grey28", indicatoron=True).grid(row=0, column=2, padx=5, pady=5)

        tk.Checkbutton(cb_frame, text="Dropped", variable=self.var_dropped,
                       font=("Arial", 12), bg="grey14", fg="snow",
                       activebackground="grey28", activeforeground="snow",
                       selectcolor="grey28", indicatoron=True).grid(row=0, column=3, padx=5, pady=5)

        tk.Checkbutton(cb_frame, text="Include Images", variable=self.var_load_images,
                       font=("Arial", 12), bg="grey14", fg="snow",
                       activebackground="grey28", activeforeground="snow",
                       selectcolor="grey28", indicatoron=True).grid(row=0, column=4, padx=5, pady=5)

        # (Note: The warning label has been removed.)
    ...
    def build_query(self, media_type, status):
        # Build a GraphQL query with the given media type and status.
        return f'''
        query ($name: String) {{
    ...
    def get_selected_statuses(self, media_type):
        # Collect the statuses based on the checkboxes.
        statuses = []
        if self.var_completed.get():
            statuses.append("COMPLETED")
        if self.var_watching.get():
            # For anime, "Watching" maps to CURRENT; for manga, map it to READING.
            statuses.append("CURRENT" if media_type == "ANIME" else "READING")
        if self.var_paused.get():
            statuses.append("PAUSED")
        if self.var_dropped.get():
    ...
    def fetch_user_list(self, username, media_type):
        ...
        if not statuses:
            self.parent.after(0, lambda: self.progress_label.config(text="No list statuses selected!"))
            return []
        results = []
        for status in statuses:
            ...
            try:
                ...
                logging.error("Error fetching %s list for '%s' with status %s: %s", media_type, username, status, e)
                continue
            seen = set()
                ...
                for entry in lst.get("entries", []):
                    ...
                    if media:
                        ...
                        if mid not in seen:
                            ...
                            results.append((mid, title, cover, score))
        results.sort(key=lambda x: x[3], reverse=True)
        new_results = []
        for score, group in groupby(results, key=lambda x: x[3]):
            group_list = list(group)
            random.shuffle(group_list)
            new_results.extend(group_list)
        return new_results
    ...
    def thread_fetch(self, media_type):
        ...
        self.parent.after(0, lambda: (self.update_progress(), self.update_comparison_gui((self.low_index + self.high_index) // 2)))
    ...
    def update_progress(self):
        ...
        later_estimate = sum(math.ceil(math.log2(len(self.sorted_list) + i + 1)) for i in range(remaining_items)) if remaining_items > 0 else 0
        total_estimated_remaining = current_estimate + later_estimate
        self.progress_label.config(
            text=(
                f"Inserted {inserted} of {total}. Remaining: {remaining_items} items "
        ...
        # Only clear unused images every 10 comparisons to avoid UI lag.
        if self.current_index != 0 and self.current_index % 10 == 0:
            self.parent.after_idle(self.clear_unused_images)
    ...
    def update_comparison_gui(self, mid):
        ...
        if self.var_load_images.get():
            new_img_obj = self.get_resized_image(new_item, btn_pixel_width, avail_height - 50)
            cand_img_obj = self.get_resized_image(candidate, btn_pixel_width, avail_height - 50)
            new_img = ImageTk.PhotoImage(new_img_obj)
            cand_img = ImageTk.PhotoImage(cand_img_obj)
            ...
            # Without images, just show the titles centered.
            self.comp_button1.config(
                image="",
                text=new_title,
                compound="center",
    ...
    def clear_unused_images(self):
        # Clear images for unsorted entries that have already been processed.
        for entry in self.unsorted_list[:self.current_index]:
            media_id = entry[0]
            if media_id in self.image_cache:
                del self.image_cache[media_id]
            keys = [key for key in self.resized_image_cache if key[0] == media_id]
            for k in keys:
                del self.resized_image_cache[k]
        # Clear images for sorted entries that are outside the current binary search range.
        for idx, entry in enumerate(self.sorted_list):
            if idx < self.low_index or idx > self.high_index:
                media_id = entry[0]
                if media_id in self.image_cache:
                    del self.image_cache[media_id]
                keys = [key for key in self.resized_image_cache if key[0] == media_id]
